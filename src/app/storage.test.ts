...
describe("saved Macs", () => {
  ...
  it("saves and reloads a Mac", () => {
    saveMac(kv, { daemon: "D1", name: "Mac One" });
    expect(loadMacs(kv)).toEqual([{ daemon: "D1", name: "Mac One" }]);
  });

  it("deduplicates by daemon id (last write wins)", () => {
    saveMac(kv, { daemon: "D1", name: "Old" });
    const macs = saveMac(kv, { daemon: "D1", name: "New" });
    expect(macs).toHaveLength(1);
    expect(macs[0]?.name).toBe("New");
  });

  it("removes by daemon id", () => {
    saveMac(kv, { daemon: "D1", name: "One" });
    saveMac(kv, { daemon: "D2", name: "Two" });
